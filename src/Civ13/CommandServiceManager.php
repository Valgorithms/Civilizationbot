    ...
    */
    ...
    private function loadDefaultHelpCommand(): void
    {
        $help = [
            ...
            'message_handler' => new MessageHandlerCallback(function (Message $message, array $message_filtered, string $command_name): PromiseInterface {
                if (! $desired_command_name = trim(substr($message_filtered['message_content_lower'], strlen($command_name)))) {
                    return $message->reply($this->getHelpMessageBuilder());
                }
                if (isset($this->guild_commands[$message->guild_id]) && $this->guild_commands[$message->guild_id] && isset($this->guild_commands[$message->guild_id][$desired_command_name]) && $this->guild_commands[$message->guild_id][$desired_command_name]) {
                    return $message->reply($this->getHelpString($message->guild_id, $desired_command_name));
                }
                if (isset($this->global_commands[$desired_command_name])) {
                    return $message->reply($this->getHelpString(null, $desired_command_name));
                }
    ...
    private function loadCommands(): void
    {
        foreach ($this->populateCommands() as $command) {
            ...
            foreach ($command['guilds'] as $guild_id) {
                ...
                //$names = array_merge($names, isset($command['alias']) ? $command['alias'] : []);
                foreach ($names as $name) {
                    $this->guild_commands[$guild_id][$name] = $command;
                }
            }
        }
    }
    ...
    private function setupMessageCommands(): void
    {
        foreach ($this->global_commands as $global_command) {
            $this->createMessageCommand($global_command);
        }
    ...
    private function isUnique($command): bool
    {
        ...
        //$names = array_merge($names, isset($command['alias']) ? $command['alias'] : []);
        foreach ($names as $name) {
            if (isset($this->global_commands[$name])) {
                return false;
            }
        }
    ...
    private function setupInteractionCommands(): void
    {
        if ($this->global_commands) {
            $this->discord->application->commands->freshen()->then(function (GlobalCommandRepository $commands): void {
                foreach ($this->global_commands as $command) {
                    if ($this->validateCommand($command)) {
                        if (str_starts_with($command['name'], '/')) {
                            continue;
                        } // Skip slash commands for now
                        if (! $commands->get('name', $command['name'])) {
                            $this->save($commands, $command['interaction_definer']);
                        }
                        $this->listenCommand($command['name'], $command('interaction_handler'));
                    }
                }
                $this->logger->debug('[GLOBAL APPLICATION COMMAND LIST]'.PHP_EOL.'`'.implode('`, `', array_map(function ($command) {
                    return $command['name'];
                }, $this->global_commands)).'`');
            });
        }
        if ($this->guild_commands) {
            foreach (array_keys($this->guild_commands) as $key) {
                if ($guild = $this->discord->guilds->get('id', $key)) {
                    $guild->commands->freshen()->then(function (GuildCommandRepository $commands) use ($key) {
                        foreach ($this->guild_commands[$key] as $command) {
                            if ($this->validateCommand($command)) {
                                if (! $commands->get('name', $command['name'])) {
                                    $this->save($commands, $command['interaction_definer']);
                                }
                        ...
                        foreach (array_keys($this->guild_commands) as $guild_id) {
                            $this->logger->debug("[GUILD APPLICATION COMMAND LIST FOR GUILD `$guild_id`]".PHP_EOL.'`'.implode('`, `', array_map(function ($command) {
                                return $command['name'];
                            }, $this->guild_commands[$guild_id])).'`');
                        }
                    });
                }
            }
    ...
    {
        foreach ($this->global_commands as $global_command) {
            if ($this->validateCommand($global_command)) {
                $this->createHTTPCommand($global_command);
            }
        }
        foreach ($this->guild_commands as $guild_command) {
            if ($this->validateCommand($guild_command)) {
                $this->createHTTPCommand($guild_command);
            }
        }
    }
    ...
    public function getHelpMessageBuilder(?string $guild_id = null, ?string $command = null, ?MessageBuilder $messagebuilder = new MessageBuilder()): MessageBuilder
    {
        if ($embed = $this->getHelpEmbed($guild_id, $command)) {
            return $messagebuilder->addEmbed($embed->setFooter($this->civ13->embed_footer));
        }
    ...
    public function getGlobalHelpString(?string $command_name = null, ?string $help = ''): string
    {
        ...
        if (isset($this->global_commands[$command_name]) && $command = $this->global_commands[$command_name]) {
            return $help .= "`{$command['name']}` - {$command['general_usage']}".PHP_EOL;
        }
        $help .= '# Global Commands'.PHP_EOL;
        foreach ($this->global_commands as $command) {
            if (isset($command['general_usage'])) {
                $help .= "`{$command['name']}` - {$command['general_usage']}".PHP_EOL;
            }
        }
    ...
    public function getGuildHelpString(?string $guild_id = null, ?string $command_name = null, ?string $help = '', ?Guild $guild = null): string
    {
        ...
        if ($guild_id && ! $guild = $this->discord->guilds->get('id', $guild_id)) {
            return $help;
        }
        $help .= '# Guild Commands'.PHP_EOL;
        if ($guild && isset($this->guild_commands[$guild_id]) && $this->guild_commands[$guild_id]) {
            if ($command_name && $command = $this->guild_commands[$command_name]) {
                return $help .= "`{$command['name']}` - {$command['general_usage']}".PHP_EOL;
            }
            foreach ($this->guild_commands[$guild_id] as $command) {
                if (isset($command['general_usage'])) {
                    $help .= "`{$command['name']}` - {$command['general_usage']}".PHP_EOL;
                }
            }
        ...
        foreach (array_keys($this->guild_commands) as $guild_command_id) {
            if (! $guild = $this->discord->guilds->get('id', $guild_command_id)) {
                continue;
            }
            if (! $this->guild_commands[$guild_command_id]) {
                continue;
            }
            $help .= "__{$guild->name} ({$guild_command_id})___".PHP_EOL;
            foreach ($this->guild_commands[$guild_command_id] as $command) {
                if (isset($command['general_usage'])) {
                    $help .= "`{$command['name']}` - {$command['general_usage']}".PHP_EOL;
                }
            }
