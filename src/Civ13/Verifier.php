...
class Verifier
{
    ...
    public function process(string $ckey, string $discord_id, ?Member $m = null): string
    {
        ...
        if ($this->verified->get('discord', $discord_id)) {
            if (($member = $this->civ13->discord->guilds->get('id', $this->civ13->civ13_guild_id)->members->get('id', $discord_id)) && ! $member->roles->has($this->civ13->role_ids['infantry']) && ! $member->roles->has($this->civ13->role_ids['veteran'])) $member->setRoles([$this->civ13->role_ids['infantry']], "approveme join $ckey");
    ...
    public function verifierStatusTimer(): TimerInterface
    {
        if (! isset($this->timers['verifier_status_timer'])) $this->timers['verifier_status_timer'] = $this->discord->getLoop()->addPeriodicTimer(1800, function () {
            if (! $status = $this->civ13->verifier_online) {
                ...
                if ($status !== $this->civ13->verifier_online) foreach ($this->provisional as $ckey => $discord_id) $this->provisionalRegistration($ckey, $discord_id); // If the verifier was offline, but is now online, reattempt registration of all provisional users
            }
        });
