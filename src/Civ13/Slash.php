    ...
    private function __declareListeners(): void
    {
        ...
        $this->discord->listenCommand('unverify', function (Interaction $interaction): PromiseInterface
        {
            ...
            return $interaction->acknowledge()->then(function () use ($interaction, $item) { // wait until the bot says "Is thinking..."
                ...
                return $this->sendFollowUpMessage($interaction, Civ13::createBuilder()->setContent("Unverifying `{$item['ss13']}`..."))->then(function ($message) use ($interaction, $item) {
                    $content = $this->civ13->verifier->unverify($item['ss13']);
                    $interaction->updateOriginalResponse(Civ13::createBuilder()->setContent("Processed request to unverify `{$item['ss13']}`."));
