...
class Insult extends Civ13MessageCommand
{
    ...
    {
        if (! $insults_array = file(Civ13::insults_path, FILE_IGNORE_NEW_LINES)) {
            return $this->civ13->reply($message, 'No insults found!');
        }
        if (! ($split_message = explode(' ', $message_filtered['message_content'])) || count($split_message) <= 1 || strlen($split_message[1]) === 0) {
            $split_message[1] = "<@{$message->user_id}>";
        } // $split_target[1] is the target of the insult
