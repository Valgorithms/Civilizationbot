...
class Tests extends Civ13MessageCommand
{
    public function __invoke(Message $message, string $command, array $message_filtered): PromiseInterface
    {
        ...
        if (empty($tokens[0])) {
            if (empty($this->civ13->tests)) {
                return $this->civ13->reply($message, 'No tests have been created yet! Try creating one with `tests add {test_key} {question}`');
            }
        ...
        if (! isset($tokens[1])) {
            return $this->civ13->reply($message, 'Invalid format! You must include the name of the test, e.g. `tests list {test_key}.');
        }
        if (! isset($this->civ13->tests[$test_key = strtolower($tokens[1])]) && $tokens[0] !== 'add') {
            return $this->civ13->reply($message, "Test `$test_key` hasn't been created yet! Please add a question first.");
        }
        switch ($tokens[0]) {
            case 'list':
            ...
            case 'delete':
                if (isset($tokens[2])) {
                    return $this->civ13->reply($message, 'Invalid format! Please use the format `tests delete {test_key}`');
            ...
            case 'add':
                if (! $question = implode(' ', array_slice($tokens, 2))) {
                    return $this->civ13->reply($message, 'Invalid format! Please use the format `tests add {test_key} {question}`');
                }
            ...
            case 'remove':
                if (! isset($tokens[2]) || ! is_numeric($tokens[2])) {
                    return $this->civ13->reply($message, 'Invalid format! Please use the format `tests remove {test_key} {question #}`');
                }
                if (! isset($this->civ13->tests[$test_key][$tokens[2]])) {
                    return $this->civ13->reply($message, "Question not found in test `$test_key`! Please use the format `tests {test_key} remove {question #}`");
                }
            ...
            case 'post':
                if (! isset($tokens[2]) || ! is_numeric($tokens[2])) {
                    return $this->civ13->reply($message, 'Invalid format! Please use the format `tests post {test_key} {# of questions}`');
                }
                if (count($this->civ13->tests[$test_key]) < $tokens[2]) {
                    return $this->civ13->reply($message, "Can't return more questions than exist in a test!");
                }
