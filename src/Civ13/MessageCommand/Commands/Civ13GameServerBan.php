...
class Civ13GameServerBan extends Civ13GameServerMessageCommand
{
    public function __invoke(Message $message, string $command, array $message_filtered): PromiseInterface
    {
        ...
        if (! $message_content = self::messageWithoutCommand($command, $message_filtered)) {
            return $this->civ13->reply($message, 'Missing ban ckey! Please use the format `{server}ban ckey; duration; reason`');
        }
        if (! $split_message = explode('; ', $message_content)) {
            return $this->civ13->reply($message, 'Invalid format! Please use the format `{server}ban ckey; duration; reason`');
        }
        if (! isset($split_message[0])) {
        ...
        if (! isset($split_message[2])) {
            return $this->civ13->reply($message, 'Missing ban reason! Please use the format `ban ckey; duration; reason`');
        }
        if (! str_ends_with($split_message[2], '.')) {
            $split_message[2] .= '.';
        }
        if (strlen($split_message[2]) > $maxlen = 150 - strlen(" Appeal at {$this->civ13->discord_formatted}")) {
            return $this->civ13->reply($message, "Ban reason is too long! Please limit it to `$maxlen` characters.");
        }
