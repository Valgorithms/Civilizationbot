...
class Civ14Verify extends Civ13MessageCommand
{
    ...
    public function createContainer(Member $member): Container
    {
        ...
        $this->container->addComponent(TextDisplay::new(
            ($ip)
            ? '✅ '.self::STEP_ONE_DONE
            : '❌ '.self::STEP_ONE_TODO
        ));
        if (! $ip) {
            $this->container->addComponent(ActionRow::new()->addComponent(
    ...
    public function process(Member $member): string
    {
        if (! isset($this->civ13->ss14verifier)) { // This is already checked in createContainer, so this is just a fallback if the method is called directly.
            if (isset($this->container)) {
                $this->container->setAccentColor(self::ACCENT_COLOR_ERROR);
            }
        ...
        return await($this->civ13->ss14verifier->process($member->id)->then(
            function () use ($member) {
                if ($member->roles->has($this->civ13->role_ids['SS14 Verified'])) {
                    return self::ROLE_EXISTS;
                } // This is already checked in createContainer, so this is just a fallback if the method is called directly.
                $this->addRole($member);
                if (isset($this->container)) {
                    $this->container->setAccentColor(self::ACCENT_COLOR_SUCCESS);
                }
