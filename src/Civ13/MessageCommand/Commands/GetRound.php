...
class GetRound extends Civ13MessageCommand
{
    public function __invoke(Message $message, string $command, array $message_filtered): PromiseInterface
    {
        if (! $input = self::messageWithoutCommand($command, $message_filtered)) {
            return $this->civ13->reply($message, 'Invalid format! Please use the format: getround `game_id`');
        }
        $input = explode(' ', $input);
        if (! $rounds = $this->getRounds($game_id = $input[0])) {
            return $this->civ13->reply($message, "No data found for round `$game_id`.");
        }
    ...
    protected function getRounds(string $game_id): array
    {
        ...
        foreach ($this->civ13->enabled_gameservers as $gameserver) {
            if ($round = $gameserver->getRound($game_id)) {
    ...
    protected function createBuilder(Member $member, string $game_id, array $rounds, ?string $ckey = null): MessageBuilder
    {
        ...
        foreach ($rounds as $server => $round) {
            ...
            if ($this->civ13->hasRank($member, ['Admin'])) {
    ...
    protected function createEmbed(string|int $server, array $round, bool $high_staff = false, ?string $ckey = null): Embed
    {
        ...
        if (($players = implode(', ', array_keys($round['players']))) && strlen($players) <= 1024) {
            $embed->addFieldValues('Players ('.count($round['players']).')', $players);
        } else {
            $embed->addFieldValues('Players ('.count($round['players']).')', 'Either none or too many to list!');
        }
        if ($discord_ids = array_filter(array_map(fn ($c) => ($item = $this->civ13->verifier->get('ss13', $c)) ? "<@{$item['discord']}>" : null, array_keys($round['players'])))) {
            if (strlen($verified_players = implode(', ', $discord_ids)) <= 1024) {
                $embed->addFieldValues('Verified Players ('.count($discord_ids).')', $verified_players);
            } else {
                $embed->addFieldValues('Verified Players ('.count($discord_ids).')', 'Too many to list!');
            }
        }
    ...
    protected function interaction_log_handler(Interaction $interaction): PromiseInterface
    {
        ...
        foreach ($this->civ13->enabled_gameservers as &$gameserver) {
            $keys[] = $gameserver->key;
            if (trim($tokens[0]) !== $gameserver->key) {
                continue;
            ...
            if (count($results[1]) > 7) {
                $results[1] = [array_pop($results[1]), array_pop($results[1]), array_pop($results[1]), array_pop($results[1]), array_pop($results[1]), array_pop($results[1]), array_pop($results[1])];
            }
            if (! isset($results[2]) || ! $results[2]) {
                return $interaction->sendFollowUpMessage(Civ13::createBuilder()->setContent('Available options: '.PHP_EOL.'`'.implode('`'.PHP_EOL.'`', $results[1]).'`'));
            }
