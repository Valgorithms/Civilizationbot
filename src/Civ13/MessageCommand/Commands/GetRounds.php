...
class GetRounds extends Civ13MessageCommand
{
    public function __invoke(Message $message, string $command, array $message_filtered): PromiseInterface
    {
        if (! $ckey = self::messageWithoutCommand($command, $message_filtered, true, true)) {
            return $this->civ13->reply($message, 'Invalid format! Please use the format: getrounds `ckey`');
        }
        if (! $item = $this->civ13->verifier->getVerifiedItem($ckey)) {
            return $this->civ13->reply($message, "No verified data found for ID `$ckey`.");
        }
        $rounds = [];
        foreach ($this->civ13->enabled_gameservers as $gameserver) {
            if ($r = $gameserver->getRounds([$item['ss13']])) {
                $rounds[$gameserver->name] = $r;
            }
        }
        if (! $rounds) {
            return $this->civ13->reply($message, 'No data found for that ckey.');
        }
