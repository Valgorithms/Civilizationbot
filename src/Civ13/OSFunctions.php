...
class OSFunctions
{
    ...
    private static function readPipes(array &$pipes)
    {
        ...
        if (is_resource($pipes[1]) && $stdout = fread($pipes[1], 4096)) {
            error_log("StdOut: $stdout");
        }
        if (is_resource($pipes[2]) && $stderr = fread($pipes[2], 4096)) {
    ...
    public static function spawnChildProcess(string $cmd): PromiseInterface
    {
        if (PHP_OS_FAMILY == 'Windows') {
            return reject(new \Exception('Windows does not support POSIX compatibility layer'));
        }
    ...
    {
        if (PHP_OS_FAMILY == 'Windows') {
            if (($p = popen("start {$cmd}", 'r')) === false) {
                return reject(new MissingSystemPermissionException('popen() failed'));
            }

            return resolve($p);
        }
        if (! $proc = proc_open($cmd, self::DEFAULT_PIPES, $pipes)) {
            return reject(new MissingSystemPermissionException('proc_open() failed'));
        } // old method was "sudo nohup $cmd > /dev/null &"
        ...
        if (! $proc_details = proc_get_status($proc)) {
            return reject(new MissingSystemPermissionException('proc_get_status() failed'));
        }
        if (! isset($proc_details['pid']) || ! $pid = $proc_details['pid']) {
            return reject(new MissingSystemPermissionException('proc_get_status() did not return a PID'));
        }
    ...
    {
        if (PHP_OS_FAMILY == 'Windows') {
            if (($p = popen('cmd /c "'.getcwd().'\run.bat"', 'r')) === false) {
                return reject(new MissingSystemPermissionException('popen() failed'));
            }

            return resolve($p);
        }
        if (! $proc = proc_open($output = "sudo nohup php \"$file\" &", self::DEFAULT_PIPES, $pipes)) {
            return reject(new MissingSystemPermissionException('proc_open() failed'));
        } // old method was "sudo nohup $cmd > /dev/null &"
        foreach ($pipes as $pipe) {
            if (is_resource($pipe)) {
    ...
    public static function portIsAvailable(int $port = 1714): PromiseInterface
    {
        if (($s = socket_create(AF_INET, SOCK_STREAM, SOL_TCP)) === false) {
            return reject(new \Exception(strval(socket_last_error())));
        }
        if (socket_bind($s, '127.0.0.1', $port) === false) {
            return reject(new \Exception(strval(socket_last_error($s))));
        }
        socket_close($s);
