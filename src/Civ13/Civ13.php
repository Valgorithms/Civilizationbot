...
class Civ13
{
    public const maps = '/code/__defines/maps.dm'; // Found in the cloned git repo, (e.g. '/path/to/civ13-git/code/__defines/maps.dm')

    public const log_basedir = '/data/logs';
    public const playernotes_basedir = '/data/player_saves';
    
    public const discord2ooc = '/SQL/discord2ooc.txt';
    public const discord2admin = '/SQL/discord2admin.txt';
    public const discord2dm = '/SQL/discord2dm.txt';
    public const discord2ban = '/SQL/discord2ban.txt';
    public const discord2unban = '/SQL/discord2unban.txt';
    public const admins = '/SQL/admins.txt';
    public const whitelist = '/SQL/whitelist.txt';
    public const bans = '/SQL/bans.txt';
    public const playerlogs = '/SQL/playerlogs.txt';
    public const factionlist = '/SQL/factionlist.txt';
    public const sportsteams = '/SQL/sports_teams.txt';
    public const awards = '/SQL/awards.txt';
    public const awards_br = '/SQL/awards_br.txt';

    public const updateserverabspaths = '/scripts/updateserverabspaths.py';
    public const serverdata = '/serverdata.txt';
    public const killsudos = '/scripts/killsudos.py';
    public const killciv13 = '/scripts/killciv13.py';
    public const mapswap = '/scripts/mapswap.py';

    public const dmb = '/civ13.dmb';
    public const ooc_path = '/ooc.log';
    public const asay_path = '/admin.log';
    public const ranking_path = '/ranking.txt';

    public const insults_path = 'insults.txt';
    public const status = 'status.txt';

    /** @var array<string> */
    public const array faction_teams = ['Red Faction', 'Blue Faction'];
    /** @var array<string> */
    public const array faction_admins = ['Faction Organizer'];
    ...
    public function __construct(
        ...
        if (PHP_INT_SIZE === 4 && ! BigInt::init()) {
            throw new \RuntimeException('ext-gmp is not loaded. Permissions will NOT work correctly!');
        }
        ...
        if (isset($options['discord']) && ($options['discord'] instanceof Discord)) {
            $this->discord = &$options['discord'];
        } elseif (isset($options['discord_options']) && is_array($options['discord_options'])) {
            $this->discord = new Discord($options['discord_options']);
        } else {
            $this->logger->error('No Discord instance or options passed in options!');
        }
        ...
        if (isset($options['filecache_path']) && is_string($options['filecache_path'])) {
            if (! str_ends_with($options['filecache_path'], '/')) {
                $options['filecache_path'] .= '/';
            }
            $this->filecache_path = $options['filecache_path'];
        }
        if (! is_dir($this->filecache_path)) {
            mkdir($this->filecache_path, 0664, true);
        }
        
        if (isset($options['command_symbol']) && $options['command_symbol']) {
        ...
        if (isset($options['discord_formatted'])) {
            $this->discord_formatted = $options['discord_formatted'];
        }
        if (isset($options['rules'])) {
            $this->rules = $options['rules'];
        }
        if (isset($options['gitdir'])) {
            $this->gitdir = $options['gitdir'];
        }
        if (isset($options['github'])) {
            $this->github = $options['github'];
        }
        if (isset($options['discord_invite'])) {
            $this->discord_invite = $options['discord_invite'];
        }
        if (isset($options['civ13_guild_id'])) {
            $this->civ13_guild_id = $options['civ13_guild_id'];
        }
        if (isset($options['civ_token'])) {
            $this->civ_token = $options['civ_token'];
        }
        if (isset($options['serverinfo_url'])) {
            $this->serverinfo_url = $options['serverinfo_url'];
        }
        if (isset($options['webserver_url'])) {
            $this->webserver_url = $options['webserver_url'];
        }
        if (isset($options['legacy']) && is_bool($options['legacy'])) {
            $this->legacy = $options['legacy'];
        }
        if (isset($options['moderate']) && is_bool($options['moderate'])) {
            $this->moderate = $options['moderate'];
        }
        if (isset($options['ooc_badwords']) && is_array($options['ooc_badwords'])) {
            $this->ooc_badwords = $options['ooc_badwords'];
        }
        if (isset($options['ic_badwords']) && is_array($options['ic_badwords'])) {
            $this->ic_badwords = $options['ic_badwords'];
        }

        if (isset($options['minimum_age']) && is_string($options['minimum_age'])) {
            $this->minimum_age = $options['minimum_age'];
        }
        if (isset($options['blacklisted_regions']) && is_array($options['blacklisted_regions'])) {
            $this->blacklisted_regions = $options['blacklisted_regions'];
        }
        if (isset($options['blacklsited_countries']) && is_array($options['blacklisted_countries'])) {
            $this->blacklisted_countries = $options['blacklisted_countries'];
        }
        
        if (isset($options['functions'])) {
            foreach (array_keys($options['functions']) as $key1) {
                foreach ($options['functions'][$key1] as $key2 => $func) {
                    $this->functions[$key1][$key2] = $func;
                }
            }
        } else {
            $this->logger->warning('No functions passed in options!');
        }
        
        if (isset($options['folders'])) {
            foreach ($options['folders'] as $key => $path) {
                $this->folders[$key] = $path;
            }
        } else {
            $this->logger->warning('No folders passed in options!');
        }
        if (isset($options['files'])) {
            ...
            $this->logger->warning('No files passed in options!');
        }
        if (isset($options['channel_ids'])) {
            ...
            $this->logger->warning('No channel_ids passed in options!');
        }
        if (isset($options['role_ids'])) {
            ...
            $this->logger->warning('No role_ids passed in options!');
        }
    ...
    private function afterConstruct(
        ...
        foreach ($civ13_server_settings as $gameserver_options) {
            new GameServer($this, $gameserver_options);
        }
        foreach ($civ14_server_settings as $gameserver_options) {
            new SS14GameServer($this, $gameserver_options);
        }
    ...
    private function resolveOptions(array &$options = []): array
    {
        ...
        if (! $onFulfilledDefaultValid) {
            $this->onFulfilledDefault = function ($result) {
                ...
                if (is_object($result)) {
                    ...
                    if (isset($result->scriptData)) {
                        $output .= " and scriptData of: `{$result->scriptData}`";
                    }
                    $output .= PHP_EOL;
                    ob_start();
                    var_dump($result);
        ...
        if (! $onRejectedDefaultValid) {
            $this->onRejectedDefault = function (\Throwable $reason): void {
        ...
        foreach ($options['folders'] as $key => $value) {
            if (! is_string($value) || (! is_dir($value) && ! @mkdir($value, 0664, true))) {
        ...
        foreach ($options['role_ids'] as $key => $value) {
            if (! is_numeric($value)) {
    ...
    private function __loadOrInitializeVariables(): void
    {
        ...
        if (! $this->serverinfo_url) {
            $this->serverinfo_url = "http://{$this->webserver_url}/servers/serverinfo.json";
        } // Default to VZG unless passed manually in config
    ...
    {
        if (! $message->guild || $message->guild->owner_id !== $this->owner_id) {
            return ['message_content' => '', 'message_content_lower' => '', 'called' => false];
        } // Only process commands from a guild that Taislin owns
    ...
    public function hasRequiredConfigRoles(array $required_roles = []): bool
    {
        ...
        if ($diff = array_diff($required_roles, array_keys($this->role_ids))) {
            $this->logger->error('Required roles are missing from the `role_ids` config', $diff);
    ...
    public function run(): void
    {
        ...
        if (! (isset($this->discord))) {
            $this->logger->warning('Discord not set!');
        }
    }
    /**
    ...
    public function stop(bool $closeLoop = true): void
    {
        ...
        if (isset($this->httpServiceManager->socket)) {
            $this->httpServiceManager->socket->close();
        }
        if (isset($this->verifier_server)) {
            $this->verifier_server->close();
        }
        if (isset($this->discord)) {
    ...
    public function removeRoles(
        ...
        if (! $role_ids = array_filter(self::__rolesToIdArray($roles), fn ($role_id) => $member->roles->has($role_id))) {
            return resolve($member);
        }

        return $patch
            ? ((($new_roles = $member->roles->filter(static fn (Role $role) => ! in_array($role->id, $role_ids))) !== $member->roles)
                    ? $member->setRoles($new_roles->toArray())
                    : resolve($member))
            : array_reduce(
                $role_ids,
                fn ($promise, $role_id) => $promise->then(static fn () => $member->removeRole($role_id)),
    ...
    public function addRoles(
        ...
        return $patch
            ...
            : array_reduce(
                $role_ids,
                fn ($promise, $role_id) => $promise->then(static fn () => $member->addRole($role_id)),
    ...
    public function setRoles(
        ...
        if (! $add_roles = self::__rolesToIdArray($add_roles)) {
            return resolve($member);
        }
        if (! $remove_roles = self::__rolesToIdArray($remove_roles)) {
            return resolve($member);
        }
        ...
        foreach ($remove_roles as &$role_id) {
            if (! $member->roles->has($role_id)) {
                unset($role_id);
            }
        }
    ...
    {
        if ($roles instanceof Collection && $roles->first() instanceof Role) {
            return $roles->map(fn ($role) => $role->id)->toArray();
        }
        if (! $roles instanceof Collection && is_array($roles) && $roles[0] instanceof Role) {
    ...
    public function sendMessage(Channel|Thread|string $channel, MessageBuilder|string $content, string $file_name = 'message.txt', bool $prevent_mentions = false): PromiseInterface
    {
        ...
        if ($content instanceof MessageBuilder) {
            if ($prevent_mentions) {
                $content->setAllowedMentions(AllowedMentions::none());
            }
        ...
        if (strlen($content) <= 2000) {
            return $channel->sendMessage($builder->setContent($content))->then($this->onFulfilledDefault, $this->onRejectedDefault);
        }
        if (strlen($content) <= 4096) {
            return $channel->sendMessage($builder->addEmbed($this->createEmbed()->setDescription($content)))->then($this->onFulfilledDefault, $this->onRejectedDefault);
        }
    ...
    public function reply(Message|Thread $message, string $content, string $file_name = 'message.txt', bool $prevent_mentions = false): PromiseInterface
    {
        $builder = self::createBuilder($prevent_mentions);
        if (strlen($content) <= 2000) {
            return $message->reply($builder->setContent($content));
        }
        if (strlen($content) <= 4096) {
            return $message->reply($builder->addEmbed($this->createEmbed()->setDescription($content)));
        }
    ...
    public function OOCMessage(string $message, string $sender, string|int|null $server_key = null): bool
    {
        if (is_null($server_key)) {
            return array_reduce($this->enabled_gameservers, fn ($carry, $server) => $server->OOCMessage($message, $sender), false);
        }
    ...
    {
        if (is_null($server_key)) {
            return array_reduce($this->enabled_gameservers, fn ($carry, $server) => $server->AdminMessage($message, $sender), false);
        }
    ...
    {
        if (is_null($server_key)) {
            return array_reduce($this->enabled_gameservers, fn ($carry, $server) => $server->DirectMessage($message, $sender, $recipient), false);
        }
     ...
     */
    public function checkRolePosition(string $role_id, Guild $guild): bool
    {
        if ($role_id == $guild->id) {
            return false;
        }
        if (! $bot = $guild->members->get('id', $this->discord->id)) {
            return false;
        }
        if (! $role = $guild->roles->get('id', $role_id)) {
            return false;
        }
        foreach ($bot->roles as $brole) {
            if ($brole->position > $role->position && $brole->permissions->manage_roles) {
                return true;
            }
        }
     ...
     */
    public function comparePositionTo(Role $role, Role $role2): int
    {
        if ($role->position === $role2->position) {
     ...
     * @return ?Role
     */
    public function getHighestRole(ExCollectionInterface $roles): ?Role
    {
        return $roles->reduce(fn ($prev, $role) => ($prev === null
                ? $role
     ...
     */
    public function hasRank(Member $member, array $allowed_ranks = ['Owner', 'Chief Technical Officer', 'Ambassador']): bool
    {
    ...
    public function FileNav(string $basedir, array $subdirs): array
    {
        ...
        if (! $subdir = array_shift($subdirs)) {
            return [false, $scandir];
        }
        if (! in_array($subdir = trim($subdir), $scandir)) {
            return [false, $scandir, $subdir];
        }
        if (is_file("$basedir/$subdir")) {
            return [true, "$basedir/$subdir"];
        }
    ...
    public function bancheckTimer(): TimerInterface|false
    {
        ...
        if (! $this->enabled_gameservers) {
            return false;
        } // This function should only run if there are servers to check
        ...
        if (! isset($this->timers['bancheck_timer']) || ! $this->timers['bancheck_timer'] instanceof TimerInterface) {
            $this->timers['bancheck_timer'] = $this->discord->getLoop()->addPeriodicTimer(43200, fn () => $this->bancheckTimer());
        }
    ...
    private function __bancheckTimer(): void
    {
        ...
        if (isset($this->verifier, $this->role_ids['Banished']) && $guild = $this->discord->guilds->get('id', $this->civ13_guild_id)) {
            foreach ($guild->members as $member) {
                if (! $item = $this->verifier->getVerifiedMemberItems()->get('discord', $member->id)) {
                    continue;
                }
                if (! isset($item['ss13'])) {
                    continue;
                ...
                if (($banned = $this->bancheck($item['ss13'], true, false)) && ! ($member->roles->has($this->role_ids['Banished']) || $member->roles->has($this->role_ids['Permabanished']))) {
                    ...
                    if (isset($this->channel_ids['staff_bot']) && $channel = $this->discord->getChannel($this->channel_ids['staff_bot'])) {
                        $this->sendMessage($channel, "Added the banished role to $member.");
                    }
                    ...
                    if (isset($this->channel_ids['staff_bot']) && $channel = $this->discord->getChannel($this->channel_ids['staff_bot'])) {
                        $this->sendMessage($channel, "Removed the banished role from $member.");
                    }
                }
            }
        }
    ...
    public function bancheck(string $ckey, bool $bypass = false, bool $use_cache = false): bool
    {
        ...
        if ($banned && ! $member->roles->has($this->role_ids['Banished']) && ! $member->roles->has($this->role_ids['Admin'])) {
            $member->addRole($this->role_ids['Banished'], "bancheck ($ckey)");
        } elseif (! $banned && $member->roles->has($this->role_ids['Banished'])) {
            $member->removeRole($this->role_ids['Banished'], "bancheck ($ckey)");
        }
    ...
    public function permabancheck(string $ckey, bool $bypass = false): bool
    {
        ...
        if ($permabanned && ! $member->roles->has($this->role_ids['Permabanished']) && ! $member->roles->has($this->role_ids['Admin'])) {
            $member->setRoles([$this->role_ids['Banished'], $this->role_ids['Permabanished']], "permabancheck ($ckey)");
        } elseif (! $permabanned && $member->roles->has($this->role_ids['Permabanished'])) {
            $member->removeRole($this->role_ids['Permabanished'], "permabancheck ($ckey)");
        }
    ...
    {
        if ($state) {
            $this->paroled[$ckey] = $admin;
        } else {
            unset($this->paroled[$ckey]);
        }
    ...
    public function ban(array $array /* = ['ckey' => '', 'duration' => '', 'reason' => ''] */, ?string $admin = null, string|int|null $server = null, bool $permanent = false): string
    {
        ...
        if (is_numeric($array['ckey'])) {
            if (isset($this->verifier) && ! $item = $this->verifier->get('discord', $array['ckey'])) {
                return "Unable to find a ckey for <@{$array['ckey']}>. Please use the ckey instead of the Discord ID.";
            }
            $array['ckey'] = $item['ss13'];
        }
        if (isset($this->verifier) && $member = $this->verifier->getVerifiedMember($array['ckey'])) {
            if (! $member->roles->has($this->role_ids['Banished'])) {
                if (! $permanent) {
                    ...
                    $member->setRoles([$this->role_ids['Banished'], $this->role_ids['Permabanished']], "Banned for {$array['duration']} with the reason {$array['reason']}");
                }
            }
        }
        $return = '';
        if (is_null($server)) {
            foreach ($this->enabled_gameservers as &$gameserver) {
                $return .= $gameserver->ban($array, $admin, $permanent);
            }
        } elseif (isset($this->enabled_gameservers[$server])) {
            $return .= $this->enabled_gameservers[$server]->ban($array, $admin, $permanent);
        } else {
            $return .= 'Invalid server specified for ban.';
        }
    ...
    public function unban(string $ckey, ?string $admin = null, string|array|null $gameserver = null): PromiseInterface
    {
        ...
        if (is_null($gameserver)) {
            foreach ($this->enabled_gameservers as $gs) {
                $this->unban($ckey, $admin, $gs->key);
            }
        } elseif (isset($this->enabled_gameservers[$gameserver])) {
            $this->enabled_gameservers[$gameserver]->unban($ckey, $admin);
        } else {
        ...
        if (isset($this->verifier) && $member = $this->verifier->getVerifiedMember($ckey)) {
            ...
            if ($member->roles->has($this->role_ids['Permabanished'])) {
                $member->removeRole($this->role_ids['Permabanished'], "Unbanned by $admin")->then(fn () => $member->addRole($this->role_ids['Verified'], "Unbanned by $admin"));
            }
        }
    ...
    public function ckeyinfo(string $ckey, int $max_iterations = 10): array
    {
        ...
        if (! $collectionsArray = $this->getCkeyLogCollections($ckey)) {
            return [null, null, null, false, false];
        }
        if ($item = $this->verifier->getVerifiedItem($ckey)) {
            $ckey = $item['ss13'];
        }
        ...
        $ips = [];
        $cids = [];
        foreach (['playerlogs', 'bans'] as $type) {
            foreach ($collectionsArray[$type] as $log) {
                if (isset($log['ip']) && ! in_array($log['ip'], $ips)) {
                    $ips[] = $log['ip'];
                }
                if (isset($log['cid']) && ! in_array($log['cid'], $cids)) {
                    $cids[] = $log['cid'];
                }
            }
        }
        for ($i = 0; $i < $max_iterations; $i++) { // Iterate through the player logs and ban logs to find all known ckeys, ips, and cids
            $found_ckeys = [];
            $found_ips = [];
            $found_cids = [];
    ...
    private function __processLogs(CollectionInterface|array $logs, array &$found_ckeys, array &$found_ips, array &$found_cids, array $ckeys, array $ips, array $cids): bool
    {
        ...
        foreach ($logs as $log) {
            if (isset($ckeys_set[$log['ckey']]) || isset($ips_set[$log['ip']]) || isset($cids_set[$log['cid']])) {
    ...
    public static function updateCkeyinfoVariables(
        ...
        int $i = 0,
        bool $found = false // Passed internally within the function, not to be provided by the user
    ): void {
        // Iterate through logs to find all known ckeys, ips, and cids
        foreach ($logs as $log) {
            if (isset($ckeys[$log['ckey']]) || isset($ips[$log['ip']]) || isset($cids[$log['cid']])) {
                if (isset($log['ckey']) === $update_found_ckeys) {
                    if (isset($log['ckey']) && ! isset($ckeys[$log['ckey']], $found_ckeys[$log['ckey']])) {
                        $found_ckeys[$log['ckey']] = Civ13::sanitizeInput($log['ckey']);
                        $found = true;
                    }
                }
                if (isset($log['ip'  ]) && ! isset($ips  [$log['ip'  ]], $found_ips  [$log['ip'  ]])) {
                    $found_ips  [$log['ip'  ]] = $log['ip'  ];
                    $found = true;
                }
                if (isset($log['cid' ]) && ! isset($cids [$log['cid' ]], $found_cids [$log['cid' ]])) {
                    $found_cids [$log['cid' ]] = $log['cid' ];
                    $found = true;
                }
                if (isset($log['date']) && ! isset($dates[$log['date']], $found_dates[$log['date']])) {
                    $found_dates[$log['date']] = $log['date'];
                }
            }
        }

        if ($ckeys !== $found_ckeys) {
            $ckeys = array_unique(array_merge($ckeys, $found_ckeys));
        }
        if ($ips !== $found_ips) {
            $ips = array_unique(array_merge($ips, $found_ips));
    ...
    public function ckeyinfoEmbed(string $ckey, ?array $ckeyinfo = null): Embed
    {
        ...
        if (isset($this->verifier) && $user = $this->verifier->getVerifiedUser($ckey)) {
            $embed->setAuthor("{$user->username} ({$user->id})", $user->avatar);
        }
        if (! empty($ckeyinfo['ckeys'])) {
            $embed->addFieldValues('Ckeys', implode(', ', array_map(fn ($ckey) => isset($this->ages[$ckey]) ? "$ckey ({$this->ages[$ckey]})" : $ckey, $ckeyinfo['ckeys'])));
        }
        if (! empty($ckeyinfo['discords'])) {
            $embed->addfieldValues('Discord', implode(', ', array_map(fn ($id) => $id ? "<@{$id}>" : $id, $ckeyinfo['discords'])), true);
        }
        if (! empty($ckeyinfo['ips'])) {
            $embed->addFieldValues('IPs', implode(', ', $ckeyinfo['ips']), true);
        }
        if (! empty($ckeyinfo['cids'])) {
            $embed->addFieldValues('CIDs', implode(', ', $ckeyinfo['cids']), true);
        }
        if (! empty($ckeyinfo['ips'])) {
            $embed->addFieldValues('Regions', implode(', ', array_unique(array_map(fn ($ip) => $this->getIpData($ip)['countryCode'] ?? 'unknown', $ckeyinfo['ips']))), true);
        }
    ...
    {
        if ($allow) {
            $this->permitted[$ckey] = true;
        } else {
            unset($this->permitted[$ckey]);
        }
    ...
    {
        if ($ban) {
            $this->softbanned[$id] = true;
        } else {
            unset($this->softbanned[$id]);
        }
