...
class MessageServiceManager
{
    ...
    public function handle(Message $message): ?PromiseInterface
    {
        ...
        if ($return = $this->messageHandler->handle($message)) {
            return $return;
        }
        if (! $message_array['message_content_lower']) { // No command given
            ...
            if (count($random_responses) > 0) {
                return $this->civ13->reply($message, $random_responses[rand(0, count($random_responses) - 1)]);
            }
        }
    ...
    private function __generateGlobalMessageCommands(): void
    {
        $this->messageHandler
            ...
            ->offsetSet(
                'botlog',
                fn (Message $message, string $command, array $message_filtered): PromiseInterface => $message->reply(Civ13::createBuilder()->addFile('botlog.txt')),
                ['Owner', 'Chief Technical Officer']
            )
            ->offsetSet(
                'output',
                fn (Message $message, string $command, array $message_filtered): PromiseInterface => $message->reply(Civ13::createBuilder()->addFile('output.log')),
                ['Owner', 'Chief Technical Officer']
            )
            /*->offsetSet('restart',
            ...
            $this->messageHandler
                ->offsetSet(
                    ...
                    function (Message $message, string $command, array $message_filtered): PromiseInterface {
                        if (! $item = $this->civ13->verifier->getVerifiedItem($id = Civ13::sanitizeInput(substr($message_filtered['message_content_lower'], strlen($command))))) {
                            return $this->civ13->reply($message, "<@{$id}> is not currently verified with a byond username or it does not exist in the cache yet");
                        }
                        ...
                        if ($channel = $this->discord->getChannel($this->civ13->channel_ids['parole_logs'])) {
                            $this->civ13->sendMessage($channel, "`$ckey` (<@{$item['discord']}>) has been placed on parole by `$admin` (<@{$message->user_id}>).");
                        }
                ...
                ->offsetSet(
                    ...
                    function (Message $message, string $command, array $message_filtered): PromiseInterface {
                        ...
                        if ($channel = $this->discord->getChannel($this->civ13->channel_ids['parole_logs'])) {
                            $this->civ13->sendMessage($channel, "`$ckey` (<@{$item['discord']}>) has been released from parole by `$admin` (<@{$message->user_id}>).");
                        }
        ...
        if (isset($this->civ13->folders['typespess_path'], $this->civ13->files['typespess_launch_server_path'])) {
            $this->messageHandler->offsetSet('ts', new Commands\TypeSpess($this->civ13), ['Owner', 'Chief Technical Officer']);
        }
        if (isset($this->civ13->folders['ss14_basedir'])) {
            $this->messageHandler->offsetSet('ss14', new Commands\Civ14Host($this->civ13), ['Owner', 'Chief Technical Officer']);
        }
    ...
    private function __declareListener()
    {
        ...
        $this->civ13->discord->on('message', function (Message $message): void {
            if ($message->author->bot || $message->webhook_id) {
                return;
            } // Ignore bots and webhooks (including slash commands) to prevent infinite loops and other issues
            if (! $this->handle($message)) { // This section will be deprecated in the future
                $message_filtered = $this->civ13->filterMessage($message);
                if (! empty($this->civ13->functions['message'])) {
                    foreach ($this->civ13->functions['message'] as $func) {
                        $func($this->civ13, $message, $message_filtered);
                    }
                } // Variable functions
    ...
    { // Forward calls to the MessageHandler object (generateHelp, offsetGet, offsetSet, offsetExists, etc.)
        if (method_exists($this->messageHandler, $name)) {
            return call_user_func_array([$this->messageHandler, $name], $arguments);
        }
