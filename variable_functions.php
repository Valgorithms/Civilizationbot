...
$on_message = function ($civ13, $message)
{
    ...
    if (str_starts_with($message_content_lower, 'insult')) {
        ...
        if ((count($split_message) > 1 ) && strlen($split_message[1] > 0)) {
            ...
            if ($file = fopen($civ13->files['insults_path'], 'r')) {
            ...
            } else return $message->channel->sendMessage('Unable to access `' . $civ13->files['insults_path'] . '`');
        }
        return;
    }
    ...
    if (str_starts_with($message_content_lower, 'whitelistme')) {
        ...
        if (strlen($split_message) > 0) { // if len($split_message) > 1 and len($split_message[1]) > 0:
            ...
            if ($author_member = $message->member) {
                ...
                } else return $message->channel->sendMessage('Rejected! You need to have at least the [' . $author_guild->roles ? $author_guild->roles->offsetGet($civ13->role_ids['veteran'])->name : "Veteran" . '] rank.');
    ...
    if (str_starts_with($message_content_lower, 'unwhitelistme')) {
        ...
        if ($author_member = $message->member) {
            ...
            if ($accepted) {
                $removed = "N/A";
                $lines_array = array();
                if ($wlist = fopen($civ13->files['nomads_whitelist'], "r")) {
                    while (($fp = fgets($playerlogs, 4096)) !== false) {
                        $lines_array[] = $fp;
                    }
                    fclose($wlist);
                } else return $message->channel->sendMessage('Unable to access `' . $civ13->files['nomads_whitelist'] . '`');
                if ($count($lines_array) > 0) {
                    if ($wlist = fopen($civ13->files['nomads_whitelist'], "w")) {
                ...
                $lines_array = array();
                if ($wlist = fopen($civ13->files['tdm_whitelist'], "r")) {
                    while (($fp = fgets($playerlogs, 4096)) !== false) {
                        $lines_array[] = $fp;
                    }
                    fclose($wlist);
                } else return $message->channel->sendMessage('Unable to access `' . $civ13->files['tdm_whitelist'] . '`');
                if ($count($lines_array) > 0) {
                    if ($wlist = fopen($civ13->files['tdm_whitelist'], "w")) {
                        foreach ($lines_array as $line)
                            if (!str_contains($line, $message->member->username)) {
                                fwrite($wlist, $line);
                            } else {
    ...
    if (str_starts_with($message_content_lower, 'hostciv')) {
        ...
        if ($author_member = $message->member) {
            ...
            } else return $message->channel->sendMessage('Rejected! You need to have at least the [' . $author_guild->roles ? $author_guild->roles->offsetGet($civ13->role_ids['captain'])->name : "Captain" . '] rank.');
        } else return $message->channel->sendMessage('Error! Unable to get Discord Member class.');
        return;
    }
    ...
    if (str_starts_with($message_content_lower, 'restarttdm')) {
        ...
        if ($author_member = $message->member) {
            ...
            } else return $message->channel->sendMessage('Rejected! You need to have at least the [' . $author_guild->roles->offsetGet($civ13->role_ids['captain'])->name . '] rank.');
        } else return $message->channel->sendMessage('Error! Unable to get Discord Member class.');
        return;
    }
    ...
    if (str_starts_with($message_content_lower, 'maplist')) {
        ...
        if ($author_member = $message->member) {
            ...
            } else $message->channel->sendMessage('Rejected! You need to have at least the [' . $author_guild->roles->offsetGet($civ13->role_ids['captain'])->name . '] rank.');
    ...
    if (str_starts_with($message_content_lower, 'tdmmapswap')) {
        ...
        if ($author_member = $message->member) {
            ...
            } else return $message->channel->sendMessage('Rejected! You need to have at least the [' . $author_guild->roles->offsetGet($civ13->role_ids['knight'])->name . '] rank.');
        } else return $message->channel->sendMessage('Error! Unable to get Discord Member class.');
        return;
    }
    ...
    if (str_starts_with($message_content_lower, "banlist")) {
        ...
        } return $message->channel->sendMessage('Rejected! You need to have at least the [' . $author_guild->roles->offsetGet($civ13->role_ids['knight'])->name . '] rank.');
    }
...
$on_message2 = function ($civ13, $message)
{
    ...
    if (str_starts_with($message->content, $civ13->command_symbol . ' ')) { //Add these as slash commands?
        ...
        if (str_starts_with($message_content_lower, 'ranking')) {
            ...
            $line_array = array();
            if ($search = fopen($civ13->files['ranking_path'], "r")) {
                while (($fp = fgets($search, 4096)) !== false) {
                    $line_array[] = $fp;
                }
                fclose($search);
            } else return $message->channel->sendMessage('Unable to access `' . $civ13->files['ranking_path'] . '`');
            $topsum = 1;
            $msg = '';
...
$recalculate_ranking = function ($civ13)
{
    ...
    if ($search = fopen($civ13->files['tdm_awards_path'], "r")) {
        while(! feof($search)) {
            $medal_s = 0;
            $line = fgets($search);
            $line = trim(str_replace(PHP_EOL, "", $line)); # remove '\n' at end of line
    ...
    } else return $message->channel->sendMessage('Unable to access `' . $civ13->files['tdm_awards_path'] . '`');
    ...
    if ($search = fopen($civ13->files['ranking_path'], 'w')) {
...
$browser_post = function ($civ13, string $url, array $headers = ['Content-Type' => 'application/x-www-form-urlencoded'], array $data = [], $curl = false)
{
    ...
    if ( ! $curl && $browser = $civ13->browser) {
        return $browser->post($url, $headers, http_build_query($data));
    } else {
        $ch = curl_init(); //create curl resource
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); //return the transfer as a string
