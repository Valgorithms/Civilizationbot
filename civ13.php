...
class Civ13
{
    ...
    { // TODO: add infantry and veteran roles to all non-staff command parameters except for `approveme`
        ...
        $this->messageHandler->offsetSet('fixroles', new MessageHandlerCallback(function (Message $message, array $message_filtered, string $command) use ($banlog_update): PromiseInterface {
            ...
            if ($members = $guild->members->filter(function (Member $member) { return ! $member->roles->has($this->role_ids['veteran']) && ! $member->roles->has($this->role_ids['infantry']) && ! $member->roles->has($this->role_ids['banished']) && ! $member->roles->has($this->role_ids['permabanished']); }))
                foreach ($members as $member) if ($this->getVerifiedItem($member))
                    $member->addRole($this->role_ids['infantry'], 'fixroles');
            return $message->react("👍");
        }), ['Owner', 'High Staff']);
